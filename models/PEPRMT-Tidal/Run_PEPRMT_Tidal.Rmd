...
From Jim:
- GPP is important
Patty has been using remote sensing or enhanced vegetation index. This is what Jim has working on for my (next) paper
- There are two ways to get to NPP (GPP times constant ie minus R). Other way is predictions from MEM
- 
...
#The way the code is currently set up this needs to be exactly in the same structure as Patty's
#Column descriptions are in PEPRMT_GPP_FINAL.R
colnames(target)

unique(All_sites_master$EVI)
```
