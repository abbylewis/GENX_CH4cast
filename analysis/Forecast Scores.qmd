...
scores_format %>%
  ...
  geom_point(aes(color = model_id, shape = ref_year), size = 0.5) +
  geom_smooth(se = F, color = "black", method = 'gam', formula = y ~ s(x, k = 2)) +
  facet_grid(cols = vars(month))+
  xlab("Soil temperature (ºC above ambient)")+
...
scores_format %>%
  ...
  facet_grid(cols = vars(month))+
  xlab("Soil temperature (ºC above ambient)")+
  ylab("Forecast RMSE with 2021 minus without 2021\n(µmol CH4/m2/d; lower is better)")+
  scale_color_viridis_d()+
  theme_bw()
```
