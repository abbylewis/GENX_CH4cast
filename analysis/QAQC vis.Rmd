...
temp_difs <- chamber_temp[210000:211000,] %>%
  left_join(metadata) %>%
  ...
  group_by(time_1min) %>%
  mutate(z_temp = (AirTemp_C - mean(AirTemp_C, na.rm = T))/sd(AirTemp_C, na.rm = T))
temp_difs %>%
  ggplot(aes(x = time_1min, y = z_temp, color = miu_valve_bme == miu_valve_flux))+
...
#With all daytime data, calculate zscore
temp_difs <- chamber_temp %>%
  #Can only use daytime data for this
  filter(hour(TIMESTAMP) > 10 & hour(TIMESTAMP) < 16) %>%
  left_join(metadata) %>%
  select(TIMESTAMP, AirTemp_C, miu_valve) %>%
  rename(time_1min = TIMESTAMP,
         miu_valve_bme = miu_valve) %>%
  ...
  arrange(time_1min) %>%
  group_by(time_1min) %>%
  mutate(z_temp = (AirTemp_C - mean(AirTemp_C, na.rm = T)) /
           sd(AirTemp_C, na.rm = T))

#We're going to remove points with a zscore greater than 1 and low CO2 R2
closed <- temp_difs %>%
  filter(miu_valve_bme == miu_valve_flux) %>%
  group_by(miu_valve_bme) %>%
  mutate(closed = ifelse(z_temp > 2, T, F),
         r2 = ifelse(CO2_R2 < 0.8, T, F))
...
closed %>%
  ...
  ggh4x::facet_grid2(rows = vars(year), cols = vars(name), scales = "free_y", independent = "y")

closed_final <- temp_difs %>%
  filter(miu_valve_bme == miu_valve_flux) %>%
  mutate(closed = ifelse(z_temp > 2, T, F),
         date = as.Date(time_1min)) %>%
  filter(!is.na(closed)) %>%
  group_by(date, miu_valve_flux) %>%
  summarize(closed = sum(closed)>=2) %>%
...
temp_difs <- chamber_temp %>%
  ...
  mutate(temp_change_quantile = ecdf(temp_change)(temp_change),
         temp_quantile = ecdf(AirTemp_C)(AirTemp_C)) 
