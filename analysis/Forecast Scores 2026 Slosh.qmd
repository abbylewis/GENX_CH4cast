...
observations %>%
  ...
  summarize(sd = sd(observation, na.rm = T),
            n = n()) %>%
  filter(n>25,
         year %in% c(2025, 2026)) %>%
  mutate(mo_yr = as.Date(paste0(year, "-", month, "-01"))) %>%
...
scores_format <- scores %>%
  ...
  mutate(temp = sub(".+_e*", "", site_id),
         ...
         temp = as.numeric(temp),
         year = year(datetime)) %>%
  filter(!model_id %in% c("RW", "climatology", "RW_no2021", "climatology_no2021"))
```
...
scores_for_relative  %>%
  ...
  summarize(rmse = Metrics::rmse(observation, mean),
            n = n()) %>%
  filter(n > 25) %>%
  group_by(horizon, model_id, year, month) %>%
  filter(length(unique(rmse[site_id == "1"]))>0) %>%
  mutate(rmse_amb = unique(rmse[site_id == "1"]),
