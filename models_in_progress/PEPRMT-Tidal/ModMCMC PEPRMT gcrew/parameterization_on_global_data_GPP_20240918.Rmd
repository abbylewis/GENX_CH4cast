...
The underlying goal of this project is to evaluate the forecast ability of PEPRMT-tidal for chamber ch4 measurements.

To run this model, parameters involved in physical chemistry within PEPRMT must be parameterized.


# Goal
This script uses the package FME's function `modMCMC`, which implements a delayed rejection adaptive monte carlo markov chain algorithm, to estimate theta parameters for PEPRMT upon a multi-plot data set.
...
Estimating parameters for PEPRMT requires calculating an error term (or vector of terms) for DRAM. The data is comprised of a data frame containing the methane, daily integral error (gap filled), and random error from Eddy Covariance Towers.
...
```{r}
# #load in the data, all sites in 1 spreadsheet
